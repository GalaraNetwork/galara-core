...
The data directory is the default location where the Galara Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.galara/`
macOS    | `$HOME/Library/Application Support/Galara/`
Windows  | `%LOCALAPPDATA%\Galara\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `galara.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `galara.conf`         | User-defined [configuration settings](bitcoin-conf.md) for `galarad`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `galarad.pid`         | Stores the process ID (PID) of `galarad` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `galarad`; can be specified by `-debuglogfile` option
...
`./`               | `galara-settings.json` | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [galara.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
This table describes the files installed by Galara Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| galara.conf                                                | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
| bin/bitcoin                                                | Upstream wrapper command; calls the node and utility binaries below.        |
| bin/galara-cli                                             | Tool for making node and wallet RPC calls.                                  |
| bin/bitcoin-qt                                             | Upstream-named Galara node and wallet GUI                                   |
| bin/galara-tx                                              | Tool for creating and modifying transactions                                |
| bin/galara-util                                            | Miscellaneous utilities                                                     |
| bin/galara-wallet                                          | Galara wallet tool                                                         |
| bin/galarad                                                | Galara node and wallet daemon                                               |
...
- README and galara.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `galara-cli.exe`).
