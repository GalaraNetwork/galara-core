...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Galara
    //   new: C:\Users\Username\AppData\Local\Galara
    // macOS: ~/Library/Application Support/Galara
    // Unix-like: ~/.galara
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Galara";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Galara";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Galara";
#else
    // Unix-like
    return pathRet / ".galara";
#endif
#endif
}
